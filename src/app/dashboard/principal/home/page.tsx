  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [statsRes, activityRes, alertsRes] = await Promise.all([
        fetch('/api/principal/dashboard/stats'),
        fetch('/api/principal/dashboard/activity'),
        fetch('/api/principal/dashboard/alerts')
      ])
      ...
      {alerts.length > 0 && (
        <div className="space-y-2">
          {alerts.slice(0, 3).map((alert) => (
            <div key={alert.id} className="flex items-center gap-2 p-3 rounded-lg border border-orange-200 bg-orange-50">
